...
function Item({language, getPriceText, getPrice}: ItemProps) {
    ...
    const getTitle = (str: string, block: number) => {
    ...
    return(
        <div style={{justifyContent: 'center', display: 'flex', width: '100%', paddingTop: '64px', height: 'calc(100vh - 124px)'}}>
            
            <div style={{position: 'absolute', width: '100%', height: '100vh', zIndex: '-5', backgroundColor: `black`, left: '0', top: '0'}}></div>
            <motion.div 
            ...
            style={{position: 'fixed',top: 200, left: '2000px', zIndex: -3, width:'1000px', height: '1000px', backgroundImage: `radial-gradient(circle, ${colors.dot1} 70%, rgba(0,0,0,0) 70%)`, filter: 'blur(300px)'}}></motion.div>
            <motion.div 
            ...
            <div style={{display: 'flex', marginTop: '30px' , marginLeft: '25px', width: '1000px', backgroundColor: 'rgb(45,45,45, 0.6)', padding: '15px', borderRadius: '20px', alignSelf: 'flex-start'}}>
                
                <div style={{marginRight: '10px'}}>
                    <div style={{display: 'flex', width: '500px', height: '396px'}}>
                    <div style={{display: 'flex', flexDirection: 'column', maxHeight: '400px', overflowY: 'auto', width: '105px'}}>
                        {photos.map((item, index) => (
                            <img key={index} src={item.src} onClick={() => setCurrentPhoto(item.src)} style={{width: '75px', marginBottom: '15px', cursor: 'pointer'}} />
                        ))}
                    </div>
                    
                    <div style={{width: '411px', height: '396px', backgroundColor: 'red'}}>
                ...
                <div>
                    <div style={{width: '100%', backgroundColor: colors.primary, height: '20px', alignContent:' center'}}>
                        <p style={{ marginLeft: '10px', color: 'white', fontSize: '12px'}}>Na powitanie</p>
                    </div>

                    <p style={{fontSize: '22px', fontWeight: 'bold', color: colors.text}}>{getTitle(name, 40)}</p>
                    
                    <div style={{display: 'flex', height: '40px'}}>

                        { discount > 0 ? (
                            <p style={{fontSize: '30px', fontWeight: 'bold', color: colors.text, textDecoration: 'line-through', marginRight: '10px'}}>{getPriceText(price)}</p>
                        ) : null}
                        
                        <p style={{fontSize: '30px', fontWeight: 'bold', color: colors.text}}>{getPriceText(getPrice(price, discount))}</p>
                        
                        {discount > 0 ? (<p style={{color: 'red', marginTop: '14px', marginLeft: '10px'}}>{discount}% zniżki</p>) : null}
                        
                    </div>

                    <div style={{display: 'flex'}}>
                        <p style={{fontSize: '13px', color: colors.text}}>Cena bez udzielonej promocji: </p>
