...
function Home({setUser, addToBasket, basket, language, getPriceText, getPrice }: HomeProps){
  ...
  const displayTitle = (name: string) => {
    ...
    return (
        <div style={{display: 'flex', height: 'calc(100vh - 64px)', overflowY: 'auto', backgroundColor: colors.background}}>
            ...
            {items.map((item, index) => (
                    <div key={item.id} 
                        ...
                        <p onClick={() => navigate(`/item/${item.id}`)} style={{fontSize: '13px'}}>{displayTitle(item.title)}</p>
                        <div style={{display: 'flex', alignItems: 'center'}}>
                          <p style={{fontSize: '17px', fontWeight: 'bold'}}>{getPriceText(getPrice(item.price, item.discount))}</p>
                        ...
                        <div style={{display: 'flex'}}>
                          <div onClick={() => addToBasket({id: item.id, src: photo1s[index], title: item.title, price: item.price, discount: item.discount})} onMouseEnter={() => setHoverIndex(item.id)} onMouseLeave={() => setHoverIndex(null)} style={{display: 'flex', alignItems: 'center', justifyContent: 'center', width: '130px', height: '30px', border: `2px solid ${colors.primary}`, backgroundColor: hoverIndex === item.id ? (colors.primary) : ('white')}}>
                            <ShoppingBasket style={{fontSize: '23px', marginRight: '5px', color: hoverIndex === item.id ? ('white') : (colors.primary)}}/>
                            <p style={{fontSize: '13px'}}>{basket.some(basketItem => basketItem.id === item.id) ? ('W koszyku') : ('Do koszyka')}</p>
                          </div>
                          <div>{item.discount > 0 ? (
